...
import { Download, LayoutTemplate, Printer, AlertTriangle, X, ZoomIn, ZoomOut, RotateCcw, FileText, Check } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-slate-50 overflow-hidden font-sans text-slate-900">
      ...
      {/* Header */}
      <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 shrink-0 print:hidden z-30 shadow-sm relative">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-lg flex items-center justify-center text-white font-bold text-xl shadow-md">
            <FileText size={20} />
          </div>
          <div>
            <h1 className="font-bold text-lg tracking-tight text-slate-800 leading-tight">ATS Architect</h1>
            <span className="text-[10px] font-medium text-indigo-500 uppercase tracking-widest bg-indigo-50 px-1.5 py-0.5 rounded">Beta</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <button 
            onClick={handlePrint}
            className="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white px-5 py-2 rounded-lg text-sm font-medium transition-all shadow-md hover:shadow-lg active:scale-95"
          >
            <Printer size={16} />
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        {/* Left: Editor Panel */}
        <div className="w-full md:w-[480px] border-r border-gray-200 bg-white flex flex-col print:hidden shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-20">
          <Editor 
            data={resumeData} 
        ...
        {/* Right: Preview Panel */}
        <div className="flex-1 bg-slate-100/50 overflow-hidden relative flex flex-col print:p-0 print:bg-white print:overflow-visible">
          
          {/* Zoom Controls (Floating) */}
          <div className="absolute bottom-6 right-8 flex items-center gap-1 bg-white p-1.5 rounded-full shadow-[0_4px_20px_rgba(0,0,0,0.08)] border border-slate-200 z-10 print:hidden transition-transform hover:scale-105">
             ...
             <span className="text-xs font-semibold text-slate-700 w-12 text-center select-none">{Math.round(zoom * 100)}%</span>
          ...
          <div className="flex-1 overflow-auto p-8 flex justify-center items-start custom-scrollbar">
            <div 
              ...
              className="w-[210mm] min-h-[297mm] bg-white shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] print:shadow-none print:w-full print:h-full print:transform-none"
            >
